...
int sys_clone(void) {
	...
	if (
		...
		argptr(2, (void *)&stack, sizeof(*arg)) < 0
		) return -1;
...
int sys_join(void) {
	...
	gwthd_t childid;
	if (
		argptr(0, (void *)&stack, sizeof(*stack)) < 0 ||
		argint(1, (int *)&childid) < 0
		) return -1;
	return join(stack, childid);
}

int sys_get_gwthd_id(void) {
	gwthd_t * id;
	if (argptr(0, (void *)&id, sizeof(*id)) < 0)
		return -1;
	return get_gwthd_id(id);
}